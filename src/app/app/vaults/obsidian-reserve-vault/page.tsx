...
export default function FirstVaultPage() {
  ...
  // Get vault config
  const vaultConfig = getVaultBySlug("obsidian-reserve-vault");
  const vaultName = vaultConfig?.name || "Obsidian Reserve Vault";
  const vaultAPY = vaultConfig?.apyNumeric || 9.3;
  const vaultTVL = vaultConfig?.tvl || "$345,678,901";
  const vaultAPYDisplay = vaultConfig?.apy || "9.3%";
