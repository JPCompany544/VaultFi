...
export default function FirstVaultPage() {
  ...
  // Get vault config
  const vaultConfig = getVaultBySlug("bitcoin-apex-vault");
  const vaultName = vaultConfig?.name || "Bitcoin Apex Vault";
  const vaultAPY = vaultConfig?.apyNumeric || 4.2;
  const vaultTVL = vaultConfig?.tvl || "$89,234,567";
  const vaultAPYDisplay = vaultConfig?.apy || "4.2%";
