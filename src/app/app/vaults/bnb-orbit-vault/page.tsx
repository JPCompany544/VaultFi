...
export default function FirstVaultPage() {
  ...
  // Get vault config
  const vaultConfig = getVaultBySlug("bnb-orbit-vault");
  const vaultName = vaultConfig?.name || "BNB Orbit Vault";
  const vaultAPY = vaultConfig?.apyNumeric || 6.8;
  const vaultTVL = vaultConfig?.tvl || "$156,789,123";
  const vaultAPYDisplay = vaultConfig?.apy || "6.8%";
