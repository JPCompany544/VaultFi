...
export default function FirstVaultPage() {
  ...
  // Get vault config
  const vaultConfig = getVaultBySlug("sentora-defi-vault");
  const vaultName = vaultConfig?.name || "Sentora DeFi Vault";
  const vaultAPY = vaultConfig?.apyNumeric || 7.2;
  const vaultTVL = vaultConfig?.tvl || "$234,567,890";
  const vaultAPYDisplay = vaultConfig?.apy || "7.2%";
