...
export default function FirstVaultPage() {
  ...
  // Get vault config
  const vaultConfig = getVaultBySlug("bitcoin-bera-vault");
  const vaultName = vaultConfig?.name || "Bitcoin Bera Vault";
  const vaultAPY = vaultConfig?.apyNumeric || 5.5;
  const vaultTVL = vaultConfig?.tvl || "$67,891,234";
  const vaultAPYDisplay = vaultConfig?.apy || "5.5%";
